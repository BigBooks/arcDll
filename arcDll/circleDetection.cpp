...
{
	/* check parameters */
	if (out == NULL) error((char*)"add_8tuple: invalid n-tuple input.");
	if (out->dim != 8) error((char*)"add_8tuple: the n-tuple must be a 8-tuple.");

	/* if needed, alloc more tuples to 'out' */
	if (out->size == out->max_size) enlarge_ntuple_list(out);
	if (out->values == NULL) error((char*)"add_8tuple: invalid n-tuple input.");
	...
	image_double* modgrad, unsigned int n_bins)
{
	...
	if (list_p == NULL) error((char*)"ll_angle: NULL point2ier 'list_p'.");
	// if( mem_p == NULL ) error("ll_angle: NULL point2ier 'mem_p'.");
	if (modgrad == NULL) error((char*)"ll_angle: NULL point2ier 'modgrad'.");
	...
	double prec)
{
	...
	if (reg_size == NULL) error((char*)"region_grow: invalid point2ier 'reg_size'.");
	if (reg_angle == NULL) error((char*)"region_grow: invalid point2ier 'reg_angle'.");
	...
	double density_th)
{
	...
	/* check parameters */
	if (reg == NULL) error((char*)"reduce_region_radius: invalid point2ier 'reg'.");
	if (reg_size == NULL)
		error((char*)"reduce_region_radius: invalid point2ier 'reg_size'.");
	if (prec < 0.0) error((char*)"reduce_region_radius: 'prec' must be positive.");
	if (rec == NULL) error((char*)"reduce_region_radius: invalid point2ier 'rec'.");
	if (used == NULL || used->data == NULL)
	...
	image_char used, image_double angles, double density_th)
{
	...
	/* check parameters */
	if (reg == NULL) error((char*)"refine: invalid point2ier 'reg'.");
	if (reg_size == NULL) error((char*)"refine: invalid point2ier 'reg_size'.");
	if (prec < 0.0) error((char*)"refine: 'prec' must be positive.");
	if (rec == NULL) error((char*)"refine: invalid point2ier 'rec'.");
	if (used == NULL || used->data == NULL)
...
{
	if (length <= 0)
		error((char*)"paired segment length less equal than 0");
	...
	if (pairedSegList->pairSeg == NULL)
		error((char*)"pairedSegmentListInit,not enough memory");
...
void freePairedSegmentList(PairedSegmentList* list)
{
	if (list == NULL || list->pairSeg == NULL)
		error((char*)"freePairedSegmentList,invalidate free");
	free(list->pairSeg);
	free(list);
	list = NULL;
}
...
void calculateGradient(double* img_in, unsigned int imgx, unsigned int imgy, image_double* mod, image_double* angles)
{
	if (img_in == NULL || imgx == 0 || imgy == 0)
		error((char*)"calculateGradient error!");
...
inline bool calcCircleParametersAndValidate(double* lines, int line_num, int first_line_ind, int second_line_ind, image_double angles, double distance_tolerance, point3d* circle)
{
	...
	*/
	...
	if (first_line_ind >= line_num || second_line_ind >= line_num)
		error((char*)"calcCircleParametersAndValidate, line index corrupt");
...
void  generateCenterCandidates(PairedSegmentList* pairSegmentList, double distance_tolerance, double*& centerCandidates, int* centerCandidates_num)
{
	...
	//}
	if (center_bins == NULL)
		error((char*)"generateCircleCandidates,not enough memory");
...
void  generateCircleCandidates(PairedSegmentList* pairSegmentList, double* centerCandidates, int centerCandidates_num, double distance_tolerance, double*& circleCandidates, int* circleCandidates_num)
{
	...
	// 为了和C#交换变量，再C#内声明足够大（200 double）的数组直接传进来
...
extern "C" CIRCLEDETECTION_API void generateCircleCandidates(Mat * inputImg, double* candidates, unsigned char* edgeimg_out, double* gradient_vec_out, int& candidates_num, int& edge_pixels_total_num)
{
	...
	if (pairseglist != NULL)
	{
		//printf("Number of effective line segments: %i \n", pairseglist->length);
		double* centers;
		int      centers_num;
		generateCenterCandidates(pairseglist, distance_tolerance, centers, &centers_num);
		//printf("The number of circle centers after the mean circle center drifts and clusters: %i \n", centers_num);
		//double* candidates; //相当于candidates_out
		/*int  candidates_num;*/
		generateCircleCandidates(pairseglist, centers, centers_num, distance_tolerance, candidates, &candidates_num);
		//printf("After the mean radius drifts and clusters, the number of all candidate circle combinations: %i \n", candidates_num);
	...
	//edgeimg_out = (unsigned char*)malloc(imgy * imgx * sizeof(unsigned char));
	//这里我们在C#内生成足够大的（imgy*imgx char）传进来

	edge_pixels_total_num = 0;//边缘总像素
	...
	for (int c = 0; c < imgx; c++)
		for (int r = 0; r < imgy; r++)
		{
			...
			{
				edgeimg_out[addr] = 255;//为边缘点，赋值为白色
				//------------------------------------------------
	...
	//gradient_vec_out = (double*)malloc(2 * edge_pixels_total_num * sizeof(double));
	//申请edge_pixels_total_num x 2 来保存每一个边缘点的梯度向量，以列为优先，符合matlab的习惯
	//这里我们在C#内生成足够大的（2000 double）传进来
